...
class Veritrans_Vtdirect_PaymentController
  ...
  public function redirectAction() {
	...
	/* nedd to set payment data to Mage::getSingleton('checkout/session')->setPaymentData(); when checkout */

	$pay= Mage::getSingleton('core/session')->getPaymentData();

    $api_version = Mage::getStoreConfig('payment/vtdirect/api_version');
    $payment_type = Mage::getStoreConfig('payment/vtdirect/payment_types');
    $enable_installment = Mage::getStoreConfig('payment/vtdirect/enable_installment');
    $is_enabled_bni = Mage::getStoreConfig('payment/vtdirect/enable_installment_bni');
    $is_enabled_mandiri = Mage::getStoreConfig('payment/vtdirect/enable_installment_mandiri');
    ...
    try {
      ...
      if($redirUrl->status_code=='200' || $redirUrl->status_code=='201') {
		...
		Mage::getSingleton('core/session')->setERM($error);
