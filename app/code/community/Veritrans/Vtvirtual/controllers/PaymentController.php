 ...
 *
 * @category   Mage
 * @package    Mage_Veritrans_Vtvirtual_PaymentController
 * @author     denny
...
class Veritrans_vtvirtual_PaymentController
  ...
  // new email order
	public function send_new_order_mail($storeId, $order, $billing , $payment, $email, $name, $isGuest, $virtual=''){
		...
		if(!empty($virtual)){
			$virtual="<h4>Please complete payment with transfer method to this Permata bank virtual account number:  ".$virtual."</h4>";
		}
  ...
  public function redirectAction() {
    ...
    $api_version = Mage::getStoreConfig('payment/vtvirtual/api_version');

    Veritrans_Config::$isProduction =
        Mage::getStoreConfig('payment/vtvirtual/environment') == 'production'
        ? true : false;

    Veritrans_Config::$serverKey =
        Mage::getStoreConfig('payment/vtvirtual/server_key_v2');
    ...
    if ($current_currency != 'IDR') {
      $conversion_func = function ($non_idr_price) {
          return $non_idr_price *
              Mage::getStoreConfig('payment/vtvirtual/conversion_rate');
        };
    ...
    try {
	  ...
	  /* send an order email when redirecting to payment page although payment
       has not been completed. */
		$order->setState(Mage::getStoreConfig('payment/vtvirtual/'),true,
			'New order, waiting for payment.');
		$this->send_new_order_mail(Mage::app()->getStore()->getStoreId(), $order, $order_billing_address, $order->getPayment(), $customer_details['email'] , $order_billing_address->getName(), $order->getCustomerIsGuest(), $redirUrl->permata_va_number);
		
		$order->setEmailSent(true);
		
        // Redirected by Veritrans, if ok
        Mage::getSingleton('checkout/session')->unsQuoteId();
		$success="Veritrans success";
		Mage::getSingleton('core/session')->setSVA($redirUrl->permata_va_number);
	  ...
	  $this->cancelAction();
	  var_dump("<pre> virtual account",$e);die;
	  Mage_Core_Controller_Varien_Action::_redirect('checkout/onepage/failure', array('_secure'=>true));
    }
  }
