<?php
/**
 * Veritrans VT Direct cimb clicks Payment Controller
 *
 * @category   Mage
 ...
 * http://yoursite.com/vtcimb/payment/notification
 */

require_once(Mage::getBaseDir('lib') . '/veritrans-php/Veritrans.php');

class Veritrans_vtcimb_PaymentController
  ...
  public function redirectAction() {
    ...
    $api_version = Mage::getStoreConfig('payment/vtcimb/api_version');

    Veritrans_Config::$isProduction =
        Mage::getStoreConfig('payment/vtcimb/environment') == 'production'
        ? true : false;

    /** Veritrans_Config::$serverKey =
        Mage::getStoreConfig('payment/vtcimb/server_key_v2'); **/
	...
	$payloads['cimb_clicks']		 = array(
											'description' => "Order ".$orderIncrementId,
										);
    ...
    try {
      ...
      if($redirUrl->status_code=='201') {
       ...
       has not been completed. */
		$order->setState(Mage::getStoreConfig('payment/vtcimb/'),true,
		...
		$this->_redirectUrl($redirUrl->redirect_url);
      }
      else {
