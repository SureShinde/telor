...
class Unirgy_RapidFlowPro_Model_Mysql4_Category_Abstract extends Unirgy_RapidFlow_Model_Mysql4_Catalog_Abstract
{
    ...
    protected $_fieldAttributes = array
    (
    ...
    protected function _importFetchOldData( )
    {
        ...
        $select = $this->_write->select( )->from( array(
            ...
            "up" => $table."_varchar"
        ), "up.entity_id=e.entity_id and up.attribute_id={$upAttrId}", array( "value_id", "url_path" => "value" ) )->where( "up.value in (?)", $keys );
    ...
    protected function _fetchAttributeValues( $storeId, $defaults = false, $entityIds = null, $limitAttrIds = null )
    {
        ...
        foreach ( $this->_attributesByType as $type => $attrs )
        {
            ...
            foreach ( $rows as $r )
            {
                ...
                else if ( !is_array( $this->_entities[$r['entity_id']][$r['store_id']][$attrCode] ) )
                {
                    if ( $r['value'] != $this->_entities[$r['entity_id']][$r['store_id']][$attrCode] )
                    {
                        $this->_entities[$r['entity_id']][$r['store_id']][$attrCode] = array(
                            $this->_entities[$r['entity_id']][$r['store_id']][$attrCode],
                            $r['value']
                        );
                    }
                }
                else if ( !in_array( $r['value'], $this->_entities[$r['entity_id']][$r['store_id']][$attrCode] ) )
                {
                    $this->_entities[$r['entity_id']][$r['store_id']][$attrCode][] = $r['value'];
                }
            ...
            foreach ( $p as $k => $newValue )
            {
                ...
                if ( $selectable && !$empty )
                {
                    foreach ( ( array )$newValue as $i => $v )
                    {
                        ...
                        else if ( $autoCreateOptions && !empty( $attr['attribute_id'] ) && ( empty( $attr['source_model'] ) || $attr['source_model'] == "eav/entity_attribute_source_table" ) )
                        {
    ...
    protected function _importProcessDataDiff( )
    {
        ...
        foreach ( $this->_newData as $urlPath => $p )
        {
            ...
            if ( $isNew )
            {
                $this->_insertEntity[$urlPath] = array(
            ...
            foreach ( $p as $k => $newValue )
            {
                ...
                if ( $isNew && !$empty || $isValueChanged )
                {
                    ...
                    if ( !$isNew )
                    {
                        $logger->setColumn( isset( $this->_fieldsCodes[$k] ) ? $this->_fieldsCodes[$k] + 1 : 0 - 1 )->success( );
                    }
                    $isUpdated = true;
                }
            }
    ...
    protected function _importGenerateAttributeValues( )
    {
        ...
        foreach ( $this->_changeAttr as $urlPath => $p )
        {
            ...
            foreach ( $p as $k => $v )
            {
                ...
                else if ( isset( $this->_attrValueIds[$cId][0][$k] ) && !$storeId )
                {
                    $sActions = array( 0 => "D" );
                }
                else if ( $attr['is_global'] == 1 )
                ...
                else if ( $storeId && isset( $this->_attrValueIds[$cId][$storeId][$k] ) )
                {
                    $sActions[$sId] = "D";
                }
    ...
    protected function _importSaveEntities( )
    {
        ...
        foreach ( $this->_insertEntity as $urlPath => $a )
        {
            ...
            $this->_updateEntity[$cId] = array(
                "path" => $a['path'].$cId
            );
            $this->_categories[$cId] = array(
                "entity_id" => $cId,
                "path" => $a['path'].$cId,
                "url_path" => $urlPath
            );
    ...
    protected function _importSaveAttributeValues( )
    {
        ...
        foreach ( $this->_insertAttr as $type => $attrs )
        {
            ...
            foreach ( $attrs as $a )
            {
                ...
                if ( $type == "text" && 4000 < strlen( ( string )$value ) )
                {
