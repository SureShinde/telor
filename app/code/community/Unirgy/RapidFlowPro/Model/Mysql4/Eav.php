...
class Unirgy_RapidFlowPro_Model_Mysql4_Eav extends Unirgy_RapidFlow_Model_Mysql4_Abstract_Fixed
{
    ...
    protected function _importRowEAL( $row )
    {
        ...
        if ( !$exists )
        {
            $this->_write->insert( $alTable, array(
                "attribute_id" => $aId,
                "store_id" => $sId,
                "value" => $label
            ) );
        ...
        {
            $this->_write->update( $alTable, array(
                "value" => $label
    ...
    protected function _importRowEAXP( $row )
    {
        ...
        $new = array(
            ...
            "is_configurable" => isset( $row[13] ) && $row[13] !== "" ? ( int )$row[13] : false,
            "apply_to" => isset( $row[14] ) ? $row[14] : false,
            "is_visible_in_advanced_search" => isset( $row[15] ) && $row[15] !== "" ? ( int )$row[15] : false,
            "position" => isset( $row[16] ) && $row[16] !== "" ? ( int )$row[16] : false,
            "frontend_input_renderer" => isset( $row[17] ) ? $row[17] : false
        );
    ...
    protected function _importRowEAS( $row )
    {
        ...
        if ( !$exists )
        {
            $this->_write->insert( $asTable, array(
                "entity_type_id" => $etId,
        ...
        {
            $this->_write->update( $asTable, array(
                "sort_order" => $sortOrder
    ...
    protected function _importRowEAG( $row )
    {
        ...
        if ( !$exists )
        {
            $this->_write->insert( $agTable, array(
        ...
        if ( $sortOrder != $exists['sort_order'] )
        {
            $this->_write->update( $agTable, array(
                "sort_order" => $sortOrder
    ...
    protected function _importRowEASI( $row )
    {
        ...
        if ( !$exists )
        {
            $this->_write->insert( $eaTable, array(
                "entity_type_id" => $etId,
                "attribute_set_id" => $asId,
        ...
        {
            $this->_write->update( $eaTable, array(
                "attribute_group_id" => $agId,
                "sort_order" => $sortOrder
    ...
    protected function _importRowEAO( $row )
    {
        ...
        if ( !$exists )
        {
            $this->_write->insert( $oTable, array(
                "attribute_id" => $aId,
                "sort_order" => $sortOrder
            ) );
        ...
        else if ( $exists['sort_order'] != $sortOrder )
        {
            $this->_write->update( $oTable, array(
                "sort_order" => $sortOrder
        ...
        {
            $this->_write->insert( $olTable, array(
                "option_id" => $exists['option_id'],
                "store_id" => 0,
                "value" => $title
            ) );
    ...
    protected function _exportInitEA( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            "main" => $attrTable
        ), array( "attribute_code", "backend_type", "frontend_label", "frontend_input", "is_required", "is_unique" ) )->join( array(
            "et" => $etTable
        ), "et.entity_type_id=main.entity_type_id", array( "entity_type" => "entity_type_code" ) );
    ...
    protected function _exportInitEAL( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            "main" => $attrTable
        ), array( "attribute_code" ) )->join( array(
            "al" => $alTable
        ), "al.attribute_id=main.attribute_id", array( "label" => "value" ) )->join( array(
            "et" => $etTable
        ), "et.entity_type_id=main.entity_type_id", array( "entity_type" => "entity_type_code" ) )->join( array(
            "s" => $sTable
        ), "s.store_id=al.store_id", array( "store" => "code" ) )->where( "al.store_id in (?)", $this->_getStoreIds( ) );
        if ( !$this->_profile->getData( "options/export/system_attributes" ) )
        {
            $this->_select->where( "main.is_user_defined=1" );
        }
    ...
    protected function _exportInitEAX( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            "main" => $attrTable
        ), array( "attribute_code", "attribute_model", "backend_model", "backend_table", "frontend_model", "frontend_class", "source_model", "default_value", "note" ) )->join( array(
            "et" => $etTable
        ), "et.entity_type_id=main.entity_type_id", array( "entity_type" => "entity_type_code" ) );
        if ( !$this->_profile->getData( "options/export/system_attributes" ) )
        {
    ...
    protected function _exportInitEAXP( )
    {
        ...
        else
        {
            ...
            $this->_select->from( array(
                ...
                "ca" => $catAttrTable
            ), "ca.attribute_id=main.attribute_id", array( "*" ) );
        }
    ...
    protected function _exportInitEAS( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            "main" => $asTable
        ), array( "set_name" => "attribute_set_name", "sort_order" ) )->join( array(
            "et" => $etTable
        ), "et.entity_type_id=main.entity_type_id", array( "entity_type" => "entity_type_code" ) );
        $this->_exportFilterEntityType( );
    ...
    protected function _exportInitEAG( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            "main" => $agTable
        ), array( "group_name" => "attribute_group_name", "sort_order" ) )->join( array(
            "as" => $asTable
        ), "as.attribute_set_id=main.attribute_set_id", array( "set_name" => "attribute_set_name" ) )->join( array(
            "et" => $etTable
        ), "et.entity_type_id=as.entity_type_id", array( "entity_type" => "entity_type_code" ) );
    ...
    protected function _exportInitEASI( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            "main" => $eaTable
        ), array( "sort_order" ) )->join( array(
            "as" => $asTable
        ), "as.attribute_set_id=main.attribute_set_id", array( "set_name" => "attribute_set_name" ) )->join( array(
            "ag" => $agTable
        ), "ag.attribute_group_id=main.attribute_group_id", array( "group_name" => "attribute_group_name" ) )->join( array(
            "a" => $attrTable
        ), "a.attribute_id=main.attribute_id", array( "attribute_code" ) )->join( array(
            "et" => $etTable
        ), "et.entity_type_id=main.entity_type_id", array( "entity_type" => "entity_type_code" ) )->order( array( "as.sort_order", "as.attribute_set_name", "ag.sort_order", "ag.attribute_group_name", "main.sort_order", "a.attribute_code" ) );
        $this->_exportFilterEntityType( );
    ...
    protected function _exportInitEAO( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "ol" => $olTable
        ), "ol.option_id=main.option_id and ol.store_id=0", array( "option_name" => "value" ) )->join( array(
            "et" => $etTable
        ), "et.entity_type_id=a.entity_type_id", array( "entity_type" => "entity_type_code" ) );
    ...
    protected function _exportInitEAOL( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "od" => $olTable
        ), "od.option_id=main.option_id and od.store_id=0", array( "option_name" => "value" ) )->join( array(
            "ol" => $olTable
        ), "ol.option_id=main.option_id and ol.store_id<>0", array( "option_label" => "value" ) )->join( array(
            "s" => $sTable
        ), "s.store_id=ol.store_id", array( "store" => "code" ) )->where( "ol.store_id in (?)", $this->_getStoreIds( ) );
        $this->_exportFilterEntityType( "a" );
