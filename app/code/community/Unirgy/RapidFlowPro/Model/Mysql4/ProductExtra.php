...
class Unirgy_RapidFlowPro_Model_Mysql4_ProductExtra extends Unirgy_RapidFlow_Model_Mysql4_Catalog_Fixed
{
    ...
    protected function _importLink( $row, $linkType )
    {
        ...
        if ( $exists )
        {
            ...
            foreach ( $linkAttr as $code => $attr )
            {
                ...
                if ( !$r )
                {
                    if ( !$empty )
                    {
                        $this->_write->insert( $paramTable, array(
                ...
                else if ( $new[$code] != $r['value'] )
                {
                    $this->_write->update( $paramTable, array(
                        "value" => $new[$code]
                    ), "value_id={$r['value_id']}" );
                    $changed = true;
                }
        ...
        {
            $this->_write->insert( $linkTable, array(
                "product_id" => $p1,
            ...
            if ( $linkType == "super" && Mage::helper( "urapidflow" )->hasMageFeature( "table.product_relation" ) )
            {
                ...
                {
                    $this->_write->insert( $relTable, array(
                        "parent_id" => $p1,
                        "child_id" => $p2
                    ) );
                }
            ...
            foreach ( $new as $code => $value )
            {
                ...
                $this->_write->insert( $linkAttrTable."_".$linkAttr[$code]['data_type'], array(
    ...
    protected function _updateBundleOption( $pId, $title, $data )
    {
        if ( empty( $this->_bundleOptions[$pId][$title] ) )
        {
            $this->_bundleOptions[$pId][$title] = array(
                "parent_id" => $pId,
                "title" => $title
            );
        }
    ...
    protected function _importRowCCP( $row )
    {
        ...
        $position = !empty( $row[3] ) ? ( integer )$row[3] : 0;
        if ( !$category )
        {
        ...
        $exists = $this->_write->fetchRow( "select position from {$t} where category_id={$category['entity_id']} and product_id={$pId}" );
        if ( $exists )
        {
            if ( $exists['position'] != $position )
            {
                $this->_write->update( $t, array(
                    "position" => $position
                ), "category_id={$category['entity_id']} and product_id={$pId}" );
            }
            return self::IMPORT_ROW_RESULT_SUCCESS;
        }
        $this->_write->insert( $t, array(
    ...
    protected function _importRowCPBO( $row )
    {
        ...
        $pos = isset( $row[3] ) ? ( integer )$row[3] : 0;
        $type = isset( $row[4] ) ? $row[4] : "select";
        $required = isset( $row[5] ) ? ( integer )$row[5] : 0;
        ...
        else
        {
            ...
            $new['title'] = $title;
            $this->_write->insert( $bovTable, array(
    ...
    protected function _importRowCPBOL( $row )
    {
        ...
        if ( $exists )
        {
            ...
            $this->_write->update( $bovTable, array(
                "title" => $title
    ...
    protected function _importRowCPBOS( $row )
    {
        ...
        $new = array(
            "position" => isset( $row[4] ) ? ( integer )$row[4] : 0,
            "is_default" => isset( $row[5] ) ? ( integer )$row[5] : 0,
            "selection_price_type" => isset( $row[6] ) ? ( integer )$row[6] : 0,
            "selection_price_value" => isset( $row[7] ) ? ( double )$row[7] : 0,
            "selection_qty" => isset( $row[8] ) ? ( double )$row[8] : 0,
            "selection_can_change_qty" => isset( $row[9] ) ? ( integer )$row[9] : 0
        );
    ...
    protected function _updateCustomOption( $pId, $title, $data )
    {
        if ( empty( $this->_customOptions[$pId][$title] ) )
        {
            $this->_customOptions[$pId][$title] = array(
                "product_id" => $pId,
                "title" => $title
            );
        }
    ...
    protected function _updateCustomOptionSelection( $oId, $title, $data )
    {
        if ( empty( $this->_customOptionSelections[$oId][$title] ) )
        {
            $this->_customOptionSelections[$oId][$title] = array(
                "option_id" => $oId,
                "title" => $title
            );
        }
    ...
    protected function _importRowCPCO( $row )
    {
        ...
        $new = array(
            "type" => $row[3],
            "is_require" => isset( $row[4] ) ? ( integer )$row[4] : 0,
            "sku" => isset( $row[5] ) ? $row[5] : "",
            "sort_order" => isset( $row[6] ) ? ( integer )$row[6] : 0,
            "max_characters" => isset( $row[7] ) ? ( integer )$row[7] : 0,
            "file_extension" => isset( $row[10] ) ? $row[10] : "",
            "image_size_x" => isset( $row[11] ) ? ( integer )$row[11] : 0,
            "image_size_y" => isset( $row[12] ) ? ( integer )$row[12] : 0
        );
        $changed = false;
        ...
        else
        {
            ...
            $new['title'] = $title;
            $option = $this->_updateCustomOption( $pId, $title, $new );
            $this->_write->insert( $cotTable, array(
                "option_id" => $option['option_id'],
                "store_id" => 0,
                "title" => $title
            ) );
        ...
        if ( $exists )
        {
            ...
            else if ( $exists['price'] != $price || $exists['price_type'] != $priceType )
            {
                $this->_write->update( $copTable, array(
                    "price" => $price,
        ...
        {
            $this->_write->insert( $copTable, array(
                "option_id" => $option['option_id'],
                "store_id" => 0,
                "price" => $price,
                "price_type" => $priceType
    ...
    protected function _importRowCPCOL( $row )
    {
        ...
        if ( $exists )
        {
            if ( $exists['title'] != $title )
            {
                $this->_write->update( $cotTable, array(
                    "title" => $title
    ...
    protected function _importRowCPCOS( $row )
    {
        ...
        $new = array(
            ...
            "sort_order" => isset( $row[5] ) ? ( integer )$row[5] : 0
        );
        ...
        else
        {
            ...
            $this->_write->insert( $costTable, array(
        ...
        if ( $exists )
        {
            ...
            {
                $this->_write->update( $cospTable, array(
                    "price" => $price,
                    "price_type" => $priceType
        ...
        {
            $this->_write->insert( $cospTable, array(
    ...
    protected function _importRowCPCOSL( $row )
    {
        ...
        if ( $exists )
        {
            if ( $exists['title'] != $title )
            {
                $this->_write->update( $costTable, array(
                    "title" => $title
    ...
    protected function _importRowCPSA( $row )
    {
        ...
        if ( $exists )
        {
            ...
            {
                $this->_write->update( $superAttrTable, array(
                    "position" => $pos
            ...
            {
                $this->_write->update( $superLabelTable, array(
                    "value" => $label
                ), "value_id={$exists['value_id']}" );
                $changed = true;
            }
        ...
        {
            $this->_write->insert( $superAttrTable, array(
                "product_id" => $pId,
                "attribute_id" => $aId,
                "position" => $pos
            ) );
            $saId = $this->_write->lastInsertId( $superAttrTable );
            $this->_write->insert( $superLabelTable, array(
    ...
    protected function _importRowCPSAP( $row )
    {
        ...
        $new = array(
            "pricing_value" => $row[5] !== "" ? ( double )$row[5] : null,
            "is_percent" => ( integer )$row[6]
        );
        $superAttrTable = $this->_t( "catalog/product_super_attribute" );
        ...
        if ( !is_null( $exists['value_id'] ) )
        {
            ...
            else if ( $this->_isChangeRequired( $exists, $new ) )
            {
                $this->_write->update( $superPricingTable, $new, "value_id={$exists['value_id']}" );
                $changed = true;
            }
        }
        else if ( !is_null( $new['pricing_value'] ) )
        {
    ...
    protected function _importRowCPI( $row )
    {
        ...
        {
            $img = array(
                "attribute_id" => $this->_getGalleryAttrId( ),
                "entity_id" => $pId,
                "value" => $imgFilename
            );
        ...
        if ( $imgId )
        {
            ...
            if ( !$lbl )
            {
                $lbl = array(
                    ...
                    "position" => !empty( $row[4] ) && $row[4] !== "0" ? $row[4] : 1,
                    "disabled" => !empty( $row[5] ) ? $row[5] : 0
                );
                $this->_write->insert( $tv, $lbl );
                $result = self::IMPORT_ROW_RESULT_SUCCESS;
            }
            else if ( isset( $label ) && $lbl['label'] != $label || isset( $row[4] ) && $lbl['position'] != $row[4] || isset( $row[5] ) && $lbl['disabled'] != $row[5] )
            {
                $lbl = array(
    ...
    protected function _importRowCPIL( $row )
    {
        ...
        if ( !$lbl )
        {
            $lbl = array(
                ...
                "store_id" => $storeId,
                "label" => !empty( $label ) ? $label : "",
                "position" => !empty( $row[5] ) && $row[5] !== "0" ? $row[5] : 1,
                "disabled" => !empty( $row[6] ) ? $row[6] : 0
            );
            $this->_write->insert( $tv, $lbl );
            $result = self::IMPORT_ROW_RESULT_SUCCESS;
        }
        else if ( isset( $label ) && $lbl['label'] != $label || isset( $row[5] ) && $lbl['position'] != $row[5] || isset( $row[6] ) && $lbl['disabled'] != $row[6] )
        {
            $lbl = array(
                "label" => !empty( $label ) ? $label : "",
                "position" => !empty( $row[5] ) && $row[5] !== "0" ? $row[5] : 1,
    ...
    protected function _importRowCPPT( $row )
    {
        ...
        $pId = $this->_getIdBySku( $row[1] );
        $allGroups = $row[2] === "*" ? 1 : 0;
        $gId = $allGroups ? 0 : ( integer )$this->_getCustomerGroup( $this->_convertEncoding( $row[2] ), true );
        $qty = ( double )$row[3];
        $price = ( double )$row[4];
        $wId = !empty( $row[5] ) ? ( integer )$this->_getWebsiteId( $row[5], true ) : 0;
    ...
    protected function _importRowCPD( $row )
    {
        ...
        $new = array(
            "number_of_downloads" => isset( $row[4] ) ? ( integer )$row[4] : 0,
            "is_shareable" => !empty( $row[5] ) ? 1 : 0,
        ...
        if ( !$exists )
        {
            ...
            $exists = array(
                "link_id" => $this->_write->lastInsertId( )
            );
            $updated = true;
        }
        else if ( $this->_isChangeRequired( $exists, $new ) )
        ...
        {
            $this->_write->update( $tl, array(
                "title" => $title
            ), "title_id={$exists['title_id']}" );
            $updated = true;
        }
        if ( empty( $exists['price_id'] ) )
        {
            $this->_write->insert( $tp, array(
                "link_id" => $exists['link_id'],
                "website_id" => 0,
                "price" => $price
            ) );
        ...
        else if ( $exists['price'] != $price )
        {
            $this->_write->update( $tl, array(
                "price" => $price
            ), "price_id={$exists['price_id']}" );
            $updated = true;
        }
    ...
    protected function _importRowCPDS( $row )
    {
        ...
        if ( !$exists )
        {
            ...
            $this->_write->insert( $t, $new );
            $exists = array(
                "sample_id" => $this->_write->lastInsertId( )
            );
            $updated = true;
        }
        else if ( $this->_isChangeRequired( $exists, $new ) )
    ...
    protected function _renameRowCP( $row )
    {
        ...
        $this->_write->update( $this->_t( "catalog/product" ), array(
            "sku" => $newSku
        ), "entity_id={$pId}" );
        return self::IMPORT_ROW_RESULT_SUCCESS;
    }
    ...
    protected function _renameRowCPBO( $row )
    {
        ...
        $this->_write->update( $this->_t( "bundle/option_value" ), array(
            "title" => $newTitle
    ...
    protected function _fetchCategoryRow( $urlPath )
    {
        ...
        if ( !$this->_urlPaths )
        {
            ...
            $select = $this->_write->select( )->from( array(
                "e" => $t
            ), array( "entity_id", "path" ) )->join( array(
                "v" => $t."_varchar"
            ), "v.entity_id=e.entity_id", array( "url_path" => "value" ) )->where( "v.attribute_id=?", $this->_categoryUrlPathAttrId )->where( "v.store_id in (?)", array(
                0,
                $storeId
            ...
            foreach ( $rows as $r )
            {
                ...
                $this->_catIds[( integer )$r['entity_id']] = $data;
            }
        }
        if ( is_numeric( $urlPath ) )
        {
            if ( empty( $this->_catIds[( integer )$urlPath] ) )
            {
                return false;
            }
            return $this->_catIds[( integer )$urlPath];
        }
    ...
    protected function _deleteRowCPI( $row )
    {
        ...
        $imgFilename = $this->_convertEncoding( $row[2] );
        $iWild = trim( $row[2] ) == "*";
        $imgId = $this->_write->fetchCol( "select value_id from {$t} where attribute_id={$this->_getGalleryAttrId( )} and entity_id={$pId}".( !$iWild ? " and value={$this->_write->quote( $imgFilename )}" : "" ) );
        if ( !empty( $imgId ) )
        {
            $this->_write->delete( $t, $this->_write->quoteInto( "value_id in (?)", $imgId ) );
            return self::IMPORT_ROW_RESULT_SUCCESS;
        }
        return self::IMPORT_ROW_RESULT_NOCHANGE;
    }
    ...
    protected function _exportInitCCP( )
    {
        ...
        $upAttrId = $aId = $this->_getAttributeId( "url_path", "catalog_category" );
        $this->_select = $this->_read->select( )->from( array(
            "main" => $productTable
        ), array( "sku" ) )->join( array(
            "cp" => $categoryProductTable
        ), "cp.product_id=main.entity_id", array( "position" ) )->join( array(
            "cat" => $categoryTable
        ), "cp.category_id=cat.entity_id", array( ) )->join( array(
            "catup" => $categoryTable."_varchar"
        ), "catup.entity_id=cat.entity_id and catup.attribute_id={$upAttrId} and catup.value<>'' and catup.value is not null and catup.store_id=0", array( ) );
        $storeId = $this->_profile->getStoreId( );
        if ( $storeId != 0 )
        {
            $this->_select->joinLeft( array(
                "catups" => $categoryTable."_varchar"
            ), "catups.entity_id=cat.entity_id and catups.attribute_id={$upAttrId} and catups.value<>'' and catup.value is not null and catups.store_id='{$storeId}'", array( ) );
    ...
    protected function _exportLinkSelect( $linkType )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            "main" => $productTable
        ), array( "sku" => "sku" ) )->join( array(
            "l" => $linkTable
        ), "l.product_id=main.entity_id", array( ) )->join( array(
            "lp" => $productTable
        ), "lp.entity_id=l.linked_product_id", array( "linked_sku" => "sku" ) )->where( "l.link_type_id=?", $linkTypeId );
        ...
        foreach ( $attrs as $code => $r )
        {
            ...
            ), "{$alias}.link_id=l.link_id and {$alias}.product_link_attribute_id={$r['id']}", array( "value" ) );
        }
        $this->_applyProductFilter( );
    }
    ...
    protected function _exportInitCPCO( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "o" => $optionTable
        ), "o.product_id=main.entity_id", array( "type", "is_require", "option_sku" => "sku", "max_characters", "sort_order" ) )->join( array(
            "ot" => $optionTitleTable
        ), "ot.option_id=o.option_id and ot.store_id=0", array( "default_title" => "title" ) )->joinLeft( array(
            "op" => $optionPriceTable
        ), "op.option_id=o.option_id and op.store_id=0", array( "price", "price_type" ) );
        $this->_applyProductFilter( );
    ...
    protected function _exportInitCPCOL( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "o" => $optionTable
        ), "o.product_id=main.entity_id", array( ) )->join( array(
            "ot" => $optionTitleTable
        ), "ot.option_id=o.option_id and ot.store_id=0", array( "default_title" => "title" ) )->join( array(
            "otl" => $optionTitleTable
        ), "otl.option_id=o.option_id and otl.store_id<>0", array( "title" ) )->join( array(
            "s" => $storeTable
        ), "s.store_id=otl.store_id", array( "store" => "code" ) )->joinLeft( array(
            "opl" => $optionPriceTable
        ), "opl.option_id=o.option_id and opl.store_id=otl.store_id", array( "price", "price_type" ) )->where( "otl.option_id is not null or opl.option_id is not null" );
    ...
    protected function _exportInitCPCOS( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "os" => $optionSelTable
        ), "os.option_id=o.option_id", array( "selection_sku" => "sku", "sort_order" ) )->join( array(
            "ost" => $optionSelTitleTable
        ), "ost.option_type_id=os.option_type_id and ost.store_id=0", array( "selection_default_title" => "title" ) )->joinLeft( array(
            "osp" => $optionSelPriceTable
        ), "osp.option_type_id=os.option_type_id and osp.store_id=0", array( "price", "price_type" ) );
        $this->_applyProductFilter( );
    ...
    protected function _exportInitCPCOSL( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "os" => $optionSelTable
        ), "os.option_id=o.option_id", array( ) )->join( array(
            "ost" => $optionSelTitleTable
        ), "ost.option_type_id=os.option_type_id and ost.store_id=0", array( "selection_default_title" => "title" ) )->join( array(
            "ostl" => $optionSelTitleTable
        ), "ostl.option_type_id=os.option_type_id and ostl.store_id<>0", array( "title" ) )->join( array(
            "s" => $storeTable
        ), "s.store_id=ostl.store_id", array( "store" => "code" ) )->joinLeft( array(
            "ospl" => $optionSelPriceTable
        ), "ospl.option_type_id=os.option_type_id and ospl.store_id=ostl.store_id", array( "price", "price_type" ) );
        if ( $this->_getStoreIds( ) )
        {
    ...
    protected function _exportInitCPBO( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "bo" => $bundleOptionTable
        ), "bo.parent_id=main.entity_id", array( "required", "position", "type" ) )->join( array(
            "bov" => $bundleOptionValueTable
        ), "bov.option_id=bo.option_id and store_id=0", array( "default_title" => "title" ) );
        $this->_applyProductFilter( );
        $this->_exportConvertFields = array( "default_title" );
    }
    ...
    protected function _exportInitCPBOL( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "bo" => $bundleOptionTable
        ), "bo.parent_id=main.entity_id", array( ) )->join( array(
            "bov" => $bundleOptionValueTable
        ), "bov.option_id=bo.option_id and bov.store_id=0", array( "default_title" => "title" ) )->join( array(
            "bovl" => $bundleOptionValueTable
        ), "bovl.option_id=bo.option_id and bovl.store_id<>0", array( "title" => "title" ) )->join( array(
            "s" => $storeTable
        ), "s.store_id=bovl.store_id", array( "store" => "code" ) );
        if ( $this->_getStoreIds( ) )
        {
    ...
    protected function _exportInitCPBOS( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "bos" => $bundleSelectionTable
        ), "bos.option_id=bo.option_id", array( "position", "is_default", "selection_price_type", "selection_price_value", "selection_qty", "selection_can_change_qty" ) )->join( array(
            "bp" => $productTable
        ), "bp.entity_id=bos.product_id", array( "selection_sku" => "sku" ) );
        $this->_applyProductFilter( );
        $this->_exportConvertFields = array( "default_title" );
    }
    ...
    protected function _exportInitCPSA( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "sa" => $superAttrTable
        ), "sa.product_id=main.entity_id", array( "position" ) )->join( array(
            "a" => $attrTable
        ), "a.attribute_id=sa.attribute_id", array( "attribute_code" ) )->join( array(
            "sl" => $superLabelTable
        ), "sl.product_super_attribute_id=sa.product_super_attribute_id and store_id=0", array( "label" => "value" ) );
        $this->_applyProductFilter( );
    ...
    protected function _exportInitCPSAL( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "sa" => $superAttrTable
        ), "sa.product_id=main.entity_id", array( ) )->join( array(
            "a" => $attrTable
        ), "a.attribute_id=sa.attribute_id", array( "attribute_code" ) )->join( array(
            "sl" => $superLabelTable
        ), "sl.product_super_attribute_id=sa.product_super_attribute_id and sl.store_id<>0", array( "label" => "value" ) )->join( array(
            "s" => $storeTable
        ), "s.store_id=sl.store_id", array( "store" => "code" ) );
        if ( $this->_getStoreIds( ) )
        {
    ...
    protected function _exportInitCPSAP( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "sp" => $superPricingTable
        ), "sp.product_super_attribute_id=sa.product_super_attribute_id\n                ".( $w ? "and sp.website_id in ({$websiteIds})" : "" ), array( "value_index", "is_percent", "pricing_value" ) );
        if ( $w )
        {
            $this->_select->join( array(
                "w" => $websiteTable
            ), "w.website_id=sp.website_id", array( "website" => "code" ) );
        }
        $this->_applyProductFilter( );
    }
    ...
    protected function _exportInitCPSI( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "sl" => $superLinkTable
        ), "sl.parent_id=main.entity_id", array( ) )->join( array(
            "l" => $productTable
        ), "l.entity_id=sl.product_id", array( "linked_sku" => "sku" ) );
        $this->_applyProductFilter( );
    }
    ...
    protected function _exportInitCPI( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "g" => $galleryTable
        ), "g.entity_id=main.entity_id", array( "image_url" => "value" ) )->joinLeft( array(
            "gl" => $galleryStoreTable
        ), "gl.value_id=g.value_id", array( "label", "position", "disabled" ) )->where( "gl.store_id=0 or gl.store_id is null" );
        $this->_applyProductFilter( );
    ...
    protected function _exportInitCPIL( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "g" => $galleryTable
        ), "g.entity_id=main.entity_id", array( "image_url" => "value" ) )->join( array(
            "gl" => $galleryStoreTable
        ), "gl.value_id=g.value_id", array( "label", "position", "disabled" ) )->join( array(
            "s" => $storeTable
        ), "s.store_id=gl.store_id", array( "store" => "code" ) )->where( "gl.store_id<>0" );
        $this->_applyProductFilter( );
        $this->_exportConvertFields = array( "image_url", "label" );
    }
    ...
    protected function _exportInitCPPT( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "tp" => $priceTable
        ), "tp.entity_id=main.entity_id", array( "qty", "price" => "value", "all_groups" ) )->join( array(
            "cg" => $cGroupTable
        ), "cg.customer_group_id=tp.customer_group_id", array( "customer_group" => "customer_group_code" ) )->join( array(
            "w" => $websiteTable
        ), "w.website_id=tp.website_id", array( "website" => "code" ) );
        $this->_applyProductFilter( );
    ...
    protected function _exportInitCPD( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "d" => $dTable
        ), "d.product_id=main.entity_id", array( "sort_order", "max_downloads" => "number_of_downloads", "is_shareable", "link_url", "link_file", "link_type", "sample_url", "sample_file", "sample_type" ) )->join( array(
            "dt" => $dtTable
        ), "dt.link_id=d.link_id", array( "title" ) )->join( array(
            "dp" => $dpTable
        ), "dp.link_id=d.link_id", array( "price" ) )->where( "dt.store_id=0 and dp.website_id=0" );
        $this->_applyProductFilter( );
    ...
    protected function _exportInitCPDL( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "d" => $dTable
        ), "d.product_id=main.entity_id", array( ) )->join( array(
            "dt" => $dtTable
        ), "dt.link_id=d.link_id", array( "default_title" => "title" ) )->join( array(
            "dl" => $dtTable
        ), "dl.link_id=d.link_id", array( "title" ) )->join( array(
            "s" => $storeTable
        ), "s.store_id=dl.store_id", array( "store" => "code" ) )->where( "dt.store_id=0 and dl.store_id<>0" );
        $this->_applyProductFilter( );
        $this->_exportConvertFields = array( "default_title", "title" );
    }
    ...
    protected function _exportInitCPDP( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "dp" => $dpTable
        ), "dp.link_id=d.link_id", array( "price" ) )->join( array(
            "w" => $websiteTable
        ), "w.website_id=dp.website_id", array( "website" => "code" ) )->where( "dt.store_id=0 and dp.website_id<>0" );
        $this->_applyProductFilter( );
        $this->_exportConvertFields = array( "default_title" );
    }
    ...
    protected function _exportInitCPDS( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "d" => $dTable
        ), "d.product_id=main.entity_id", array( "sort_order", "sample_url", "sample_file", "sample_type" ) )->join( array(
            "dt" => $dtTable
        ), "dt.sample_id=d.sample_id", array( "title" ) )->where( "dt.store_id=0" );
        $this->_applyProductFilter( );
        $this->_exportConvertFields = array( "title" );
    }
    ...
    protected function _exportInitCPDSL( )
    {
        ...
        $this->_select = $this->_read->select( )->from( array(
            ...
            "dt" => $dtTable
        ), "dt.sample_id=d.sample_id", array( "default_title" => "title" ) )->join( array(
            "dl" => $dtTable
        ), "dl.sample_id=d.sample_id", array( "title" ) )->join( array(
            "s" => $storeTable
        ), "s.store_id=dl.store_id", array( "store" => "code" ) )->where( "dt.store_id=0 and dl.store_id<>0" );
        $this->_applyProductFilter( );
        $this->_exportConvertFields = array( "default_title", "title" );
    ...
    protected function _getLinkAttr( $linkType, $linkAttr = null, $param = null )
    {
        ...
        if ( empty( $this->_linkAttrs ) )
        {
            ...
            foreach ( $attrs as $a )
            {
                $this->_linkAttrs[$a['link_type_id']][$a['product_link_attribute_code']] = array(
